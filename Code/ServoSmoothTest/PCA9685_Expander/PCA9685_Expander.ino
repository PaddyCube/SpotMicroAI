...
ServoEasing Servo0(PCA9685_DEFAULT_ADDRESS, &Wire); // If you use more than one PCA9685 you should modify MAX_EASING_SERVOS at line 67 in ServoEasing.h
...
//    delay(1000);
    ...
    Serial.println(F("Move to 0 degree and back nonlinear in one second each using interrupts of Timer1"));
...
Servo0.setEasingType(EASE_SINE_IN_OUT);
  //  for (int i = 0; i < 3; ++i) {
        ...
        while (Servo1.isMovingAndCallYield() || Servo0.isMovingAndCallYield()) {
         ...
         Serial.println(F("Move to 180 degree and back nonlinear in one second each using interrupts of Timer1"));
        Servo1.startEaseToD(180, 2000);
        Servo0.startEaseToD(90, 2000);
        while (Servo1.isMovingAndCallYield() || Servo0.isMovingAndCallYield()) {
            ; // no delays here to avoid break between forth and back movement
        }
 Serial.println(F("Move to 90 degree and back nonlinear in one second each using interrupts of Timer1"));
